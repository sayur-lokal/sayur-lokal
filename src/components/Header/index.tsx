...
const Header = () => {
  ...
  //   const response = await fetch(`/api/products?query=${query}${categoryQuery}`);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (searchQuery.trim()) {
      const categoryParam = categoryValue !== "0" ? `&category=${categoryValue}` : '';
      router.push(`/search?query=${encodeURIComponent(searchQuery)}${categoryParam}`);
            ...
            <div className="max-w-[475px] w-full">
              <form onSubmit={handleSubmit} className="flex items-center gap-2 max-w-lg w-full">
                <div className="flex items-center">
                  <CustomSelect options={options} value={selectedCategory} onChange={setSelectedCategory} />
          ...
          <div className="flex w-full lg:w-auto items-center gap-7.5">
            <div className="hidden xl:flex items-center gap-3.5">
              ...
              <div>
                ...
                <p className="font-medium text-custom-sm text-dark">(+62) 74123-456</p>
              </div>
            </div>
