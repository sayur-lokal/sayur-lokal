...
const CartSidebarModal = () => {
  ...
  return (
    <div className={`fixed top-0 left-0 z-99999 overflow-y-auto no-scrollbar w-full h-screen bg-[#000000]/70 ease-linear duration-300 ${isCartModalOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="w-full max-w-[500px] shadow-1 bg-white px-4 sm:px-7.5 lg:px-11 relative modal-content">
          ...
          <div className="h-[66vh] overflow-y-auto no-scrollbar">
            <div className="flex flex-col gap-6">
              {/* <!-- cart item --> */}
              {cartItems.length > 0 ? cartItems.map((item, key) => <SingleItem key={key} item={item} removeItemFromCart={removeItemFromCart} />) : <EmptyCart />}
            </div>
          </div>
          ...
          <div className="border-t border-gray-3 bg-white pt-5 pb-4 sm:pb-7.5 lg:pb-11 mt-7.5 sticky bottom-0">
            ...
            <div className="flex items-center gap-4">
              <Link onClick={() => closeCartModal()} href="/cart" className="w-full flex justify-center font-medium text-white bg-green-dark py-[13px] px-6 rounded-md ease-out duration-200 hover:bg-[#1A693A]">
                View Cart
              </Link>

              <Link href="/checkout" className="w-full flex justify-center font-medium text-white bg-green-dark py-[13px] px-6 rounded-md ease-out duration-200 hover:bg-[#1A693A]">
                Checkout
              </Link>
            </div>
          </div>
